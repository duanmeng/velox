...
QueryDataReader::QueryDataReader(std::string path) : path_(std::move(path)) {
  Type::registerSerDe();
...
RowTypePtr QueryDataReader::traceDataType(
...
std::unique_ptr<common::FileInputStream> QueryDataReader::getFileInputStream(
    const std::shared_ptr<filesystems::FileSystem>& fs) const {
