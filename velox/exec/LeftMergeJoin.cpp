...
void LeftMergeJoin::initializeFilter(
...
bool LeftMergeJoin::findEndOfMatch(
...
bool LeftMergeJoin::tryAddOutputRowForLeftJoin() {
  VELOX_USER_CHECK(
...
bool LeftMergeJoin::tryAddOutputRowForRightJoin() {
...
void LeftMergeJoin::flattenRightProjections() {
...
bool LeftMergeJoin::tryAddOutputRow(
...
bool LeftMergeJoin::prepareOutput(
...
bool LeftMergeJoin::addToOutput() {
  if (isRightJoin(joinType_) || isRightSemiFilterJoin(joinType_)) {
...
bool LeftMergeJoin::addToOutputImpl() {
...
bool LeftMergeJoin::addToOutputForLeftJoin() {
  return addToOutputImpl<true>();
}

bool LeftMergeJoin::addToOutputForRightJoin() {
...
RowVectorPtr LeftMergeJoin::filterOutputForAntiJoin(
...
bool LeftMergeJoin::processDrain() {
...
RowVectorPtr LeftMergeJoin::handleRightSideNullRows() {
...
RowVectorPtr LeftMergeJoin::handleSingleSideOutput() {
...
bool LeftMergeJoin::advanceMatch() {
...
bool LeftMergeJoin::advanceMatchImpl() {
...
bool LeftMergeJoin::advanceLeftMatch() {
  return advanceMatchImpl<true>();
}

// Delegates to the template implementation for right match advancement.
bool LeftMergeJoin::advanceRightMatch() {
  return advanceMatchImpl<false>();
}

void LeftMergeJoin::clearLeftInput() {
  input_ = nullptr;
}

void LeftMergeJoin::clearRightInput() {
  rightInput_ = nullptr;
}

RowVectorPtr LeftMergeJoin::applyFilter(const RowVectorPtr& output) {
  const auto numRows = output->size();
...
void LeftMergeJoin::evaluateFilter(const SelectivityVector& rows) {
...
bool LeftMergeJoin::startDrain() {
  VELOX_CHECK(!leftHasDrained_);
  leftHasDrained_ = true;
  return true;
}

void LeftMergeJoin::finishDrain() {
  VELOX_CHECK(leftHasDrained_);
  VELOX_CHECK(rightHasDrained_);
...
void LeftMergeJoin::JoinTracker::reset() {
  resetLastVector();
