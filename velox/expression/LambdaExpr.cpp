...
class ExprCallable : public Callable {
 public:
  ...
  void apply(
    ...
    eval(rows, lambdaCtx, context->throwOnError(), result);
  ...
  void applyNoThrowError(
    ...
    auto row = createRowVector(context, wrapCapture, args, rows.end());
    EvalCtx lambdaCtx = createLambdaCtx(context, row, finalSelection);
    eval(rows, lambdaCtx, false, result);
    transformErrorVector(lambdaCtx, context, rows, elementToTopLevelRows);
  }
 ...
 private:
  void eval(
      ...
      bool throwOnError,
      VectorPtr* result) {
    ScopedVarSetter throwOnErrorScoped(
        lambdaCtx.mutableThrowOnError(), throwOnError);
